...
export class ScannerService {
  ...
  async syncPopularPets(options: { pages?: number; verifyDepth?: boolean } = {}): Promise<{
    ...
    // 1. Fetch top items sorted by popularity
    const allPopularItems: StoreItem[] = [];
    for (let page = 1; page <= pages; page++) {
      try {
    ...
    // Write all observed popular items into items table
    this.store.writeItems(allPopularItems, now);

    // 2. Identify unique pets and record trend ranks
    const uniquePets = new Map<string, StoreItem>();
    const trendRows: Array<{ petSlug: string; rank: number; observedAt: number }> = [];
    ...
    // 3. For the top unique pets, walk all age rungs to get the true 4-buyable normal price and cheapest neon price
    const itemsToAdd: StoreItem[] = [];
    const liquidityToWrite: Array<{ productId: number; salesPerWeek: number; observedAt: number }> = [];
      ...
      await Promise.all(
        chunk.map(async (pet) => {
          try {
            ...
            // Collect all normal product IDs across all age rungs
            ...
            // Collect all neon product IDs across all neon variants and age rungs
            const neonBaseKeys = Object.keys(props).filter((k) => k.startsWith('neon:'));
            const neonPids: number[] = [];
            ...
            // Fetch normal order books across all normal age rungs
            ...
            // Explicitly track variant IDs
            const baseNeonId = props['neon:false:false'];
            let baseNeonPids: number[] = [];
            ...
            // Fetch Base Neon (No Potion) order books specifically
            const baseNeonBooks = baseNeonPids.length > 0
              ? await this.interactiveApi.productOffers(baseNeonPids, { amount: 72 }).catch(() => [])
              : [];
            ...
            const baseNeonPrice = baseNeonPrices.length > 0 ? (baseNeonPrices[0] as number) : null;

            // Fetch neon order books across all neon age rungs
            const neonBooks = uniqueNeonPids.length > 0
              ? await this.interactiveApi.productOffers(uniqueNeonPids, { amount: 72 }).catch(() => [])
              : [];
            ...
            // Prioritize Base Neon (No Potion) price for crafting arbitrage:
            // 4 normal unpotted pets craft into a No-Potion Neon!
            const neonPrice = baseNeonPrice ?? (neonPrices.length > 0 ? (neonPrices[0] as number) : null);
